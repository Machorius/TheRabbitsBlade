...
class entity(object):
    ...
    def display(self,surface,scroll):
        image_to_render = None
        render_pos = [self.x / 100 * 9 - self.y / 100 * 9, self.x / 100 * 9 + self.y / 100 * 9]
        scroll = scroll.copy()
        if self.flip:
            scroll[0] -= 1
        if self.animation == None:
        ...
        if image_to_render != None:
            ...
            if self.alpha != None:
                image_to_render.set_alpha(self.alpha)
            blit_center(surface,image_to_render,(int(render_pos[0])-scroll[0]+self.offset[0]+center_x,int(render_pos[1] + self.h_pos)-scroll[1]+self.offset[1]+center_y))
